...
public class FindingMatching {
    ...
    public static void main(String[] args) {
        ...
        if (DishData.getAll().stream().anyMatch(Dish::isVegetarian)) {
          ...
          //  Optional<String> findFirst = list2.stream().parallel().filter(s -> s.startsWith("J")).findFirst();

            Optional<String> findAny = list1.stream().filter(s -> s.startsWith("D")).findAny();
            Optional<String> findFirst = list2.stream().filter(s -> s.startsWith("J")).findFirst();
          ...
          Optional<Dish> dishMinimum =   DishData.getAll().stream().min(Comparator.comparing(Dish::getCalories));
